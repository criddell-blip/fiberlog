...
- **Crew Load/Return submit can kick the user out of the app.** Reported live: doing a **Return** (truck → warehouse) — "from Braden to the warehouse" — in `CrewMovementSheet`, then tapping submit, exited the app entirely (browser navigated away, not just closed the sheet).
  ...
  - **Fix direction:** harden `backStack.js` so the synthetic-entry count is robust to StrictMode's mount/cleanup/mount (e.g., make reconcile idempotent / guard the cleanup `history.back()` so it can never drop below the entries actually owned; or coalesce the double-invoke). Verify by reproducing under `npm run dev`: My Stock → Return → submit, confirm it stays in the app.
