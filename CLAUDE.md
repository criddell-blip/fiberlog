...
- `public.users.crew_type` CHECK: `fiber_construction | splice | drop | locator | contractor | install | infrastructure | fiber_tech` — plus legacy `aerial` / `underground`, kept VALID for back-compat but no longer assignable in the UI (June 2026 merged aerial + underground → `fiber_construction`; see Recent major work). Render `crew_type` via `crewTypeLabel()` in `lib/crewTypes.js`, never raw (the merged value would print as `fiber_construction`).
