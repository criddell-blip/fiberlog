...
Shipped: crew narrow screen stack + sign-out dialog (Phase 1, `CrewApp.jsx`/`InfraCrewApp.jsx`); crew overlays/sheets with unsaved-input confirm (Phase 2a — `PartSearch`, `CrewMovementSheet`, the new-task overlays in `TaskList`/`SiteTaskList`, `AerialWorkspace`'s MST/add-pole pickers); manager tab navigation (Phase 3 — any non-home tab → Back returns to the home tab). Data-entry sheets register `useBackClose` *inside themselves* (mounted only when open → depth 1) with `opts.confirm` reading their own dirty state; the coordinator moves a layer to the top of the stack on activation so a sheet opened over a drilled-in screen receives Back first. Pending: manager sheets (Phase 2b, ~30 overlays — lower value, desktop-only), manager in-tab drill-ins (InventoryView sub-tabs, ProjectManager detail), crew wide-layout selection (Phase 4).
