...
15. **Part-first Crew Load mode** — add a mode toggle at the top of CrewMovementSheet between "📍 Pick a location" (current behavior) and "🔍 Find a part" (new). Find a part = search by name/SKU, results render each matching part as a header with rows per stock location underneath (sorted by qty desc), tapping a row sets `(otherLocationId, selectedPartId)` together and jumps to the qty step. Default for first-time open: Find a part — task-driven loadouts (the common case for crews who know what part they need but not which bin) get the more flexible flow by default; users who like the current "I'm at the warehouse, what's there?" mental model toggle once and the preference persists in component state. New helper `getAllStockGrouped()` returning every active inventory_stock row joined with parts_catalog + inventory_locations (~1500 rows for the current warehouse — fine on phones). Reuses the existing record_crew_movement RPC; no schema or backend work. ~1.5 hours; ~150 lines of UI. Adjacent: same toggle in Return flow.
