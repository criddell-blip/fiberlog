...
- `inventory_movements.occurred_at timestamptz` (nullable, no default) — the real **work/job date**, distinct from `created_at` (import/insert timestamp). Backfilled from the date token in `notes` for the two import families (fiber-jobs `[sonar_jobs:<acct>_YYYY-MM-DD_<type>]`, field-tech prose `Sonar install · YYYY-MM-DD HH:MM · …`) — migration `20260702000000_inventory_movements_occurred_at.sql` (487 rows). Consumers (reporting / Sage) should `COALESCE(occurred_at, created_at)`. **Auto-deduct rows stamp it with the passdown's `submissions.created_at`** (Aug 28 2026, migration `20260828120000_auto_deduct_occurred_at.sql`, 198 rows backfilled) — approval can lag the work by a week, and dating by approval time made Reports → Consumption / Sage disagree with Passdowns across week/month boundaries. Crew load/return rows still leave it NULL (`created_at` IS the event). Backfilling it is an `occurred_at`-only UPDATE — safe past the column-scoped `trg_inv_movement_immutable` trigger, which guards `notes` (and 12 other core fields) but not `occurred_at`; never set `notes` in the same UPDATE.
...
### Accounting destinations (`type='job_site'` — labeled **Region** in the UI)
- **UI label + stock semantics (Aug 2026):** the DB type stays `job_site` (CHECK + trigger + RPC references), but every surface renders it via `LOCATION_TYPE_LABELS` / `locationTypeLabel()` in `lib/inventory.js` → **"Region"**. Never add a private type-label map. Region quantities are **consumed material, not usable stock**: `isConsumedLocationType(type)` gates every cross-location rollup (`getStockSummary` → `total` vs `consumed`, `getStockTotalsByPart` skips them, `getPartLocations` → `totalQty` vs `consumedQty` + `isConsumed` per row). UI shows them separately ("+N in regions", "consumed into regions", CONSUMED status, warning banner when a Region is opened). The audit/count scope and Reconcile deliberately still allow a Region to be picked (owner-only corrections).
...
- **Service projects (Sep 2026).** The grant reimburses install material, not repairs, so a grant project can have a non-grant sibling `<name> - Service` (`projects.service_for_project_id` → the grant parent; one per parent, no chains) with its own trigger-created Region and one phase `Service`. **A project with a sibling IS grant-restricted** — there is no other flag. `src/lib/serviceRouting.js` is the single home of the rule (`SERVICE_JOB_TYPE_WORDS = ['fix']`, word-boundary so "Fixed Wireless" never matches): the fiber-jobs importer redirects any *Fix job (bucket + phase tag together, so Sage PROJECTID follows; a manual phase pick is never redirected), and the asset importer — whose report has no job type — matches each unit to the nearest fiber-jobs row for the account (`lib/sonarJobIndex.js`, window −2/+7 days because ONTs go out before dispatch closes the job). Unmatched units book as installs; when the fix job arrives later, the fiber-jobs importer proposes reclassing that account's grant-booked assets in the same atomic apply (**two-way check**). Crews log repair days against the Service phase. After the fact, the **owner** can Reclassify any consumption row (Activity feed / Reports → Consumption): a Region→Region `transfer` carrying `inventory_movements.reclass_of` (movements are immutable — counter-movement, never an UPDATE); `expandConsumptionRow` makes Reports show −qty on the old project / +qty on the new one, Sage exports one transfer line dated like the original (the Sage sheet warns about unexported rows dated before its window). Seeded: West Mountain - Service only; others via Projects → *Create Service sibling* or the New Project "Grant-funded" checkbox.
...
- Sites admin lives in `ProjectManager.jsx`'s project detail view — only renders for infra-style projects (0 phases + ≥1 site). Full CRUD: add / edit (rename, change type, address, notes, **move to different project**) / decommission (soft-delete via status='decommissioned'). Decommission is **accounting-only** (no movements; Aug 2026 — the in-modal recovery picker was removed): gear physically pulled from a site is logged as a field return (Receive PO → *Returned from field*) so it books onto the refurbished twin, never as new stock. Edit Site overlay also exposes two read-only drilldowns: **View tasks** (the site's tasks with name + type + status pill) and **View materials** (parts summed across all task_id-linked inventory_movements). Search + type pills render when ≥8 sites. Per-site task count badges.
...
- `entry_parts.line_note` — free-text per part line (asset tags / serials, comma-separated for qty > 1). The submit-sheet input renders only for `crew_type='infrastructure'` (`showAssetTags` in `TaskWorkspace`); draft-persisted as `working_counts.partNotes` (computed rows, keyed like `partSources`) / inline `lineNote` on extra lines. **Not part of the line identity** — `(part_id, source_location_id)` still decides how many movements a SKU becomes; when same-identity lines collapse, notes concatenate with `, ` (`joinLineNotes` in `lib/shared.js`, plus `string_agg` inside `replace_submission_parts` / `approve_submission`).
- `inventory_movements.line_note` — `approve_submission` stamps the folded lines' tags here. Separate nullable column, outside the immutable-guard list, **never appended to `notes`** (that fixed string is what the occurred_at backfill keys on). Read by the Activity feed + CSV ("Asset tags"), and by `getMaterialsAtSite` → the site's View-materials drilldown (`assetTags` per part). Sage ignores it. Migration `20260902120000_entry_parts_line_note.sql`.
...
- **Strand (July 2026)** moved off its old mechanism — a hardcoded `per_ft` assembly part on `strand-ft` — onto the map, so all three footage kinds now work identically. Cutover step 2 (drop the legacy per_ft row, `20260802120000_strand_ft_drop_per_ft_part.sql`) was applied to the live DB Aug 25 2026 — the map is now the only strand path. `STRAND_SIZES` is deliberately length 1 (`'1/4"'`, the only strand cable SKU): a kind with exactly one chip is auto-picked when feet are first entered (`autoPickType`), because otherwise strand would regress from "type feet, done" to "type feet AND tap the only chip, or nothing deducts". Adding a strand size turns that auto-pick off — there's a test tripwire on it.
...
| Manager Record movement (`RecordMovementSheet`) | transfer / return / issue / scrap / adjust (NOT receive — that's Receive PO only, Aug 2026; defaults to transfer) | any → any | Free-form, no permission filter. The former Move-stock pill (`MoveStockSheet`) was removed at the same time — this sheet covers relocation. Every line shows an On-hand summary (top 3 locations + total). Adjust's third direction **Counted total** (backlog #38, Aug 2026) takes absolute counts at one location and books per-line one-sided adjusts via `buildCountedAdjustPayloads` in `lib/inventory.js` (tested; system qtys via `getStockRowsForParts` because `getPartLocations` drops qty ≤ 0). |
...
| Manager Sonar import (`SonarImportSheet`) | `transfer` (× N lines) | crew truck → routed bucket | Destination via `parts_catalog.sonar_routing` (region / gigwave / Fixed Wireless) + `sonar_city_bucket_map`. Precedence: manual per-row pick → **wireless part policy (gigwave/none — outranks the Sonar project tag, Aug 2026; wireless CPE on "West Mountain Fiber"-tagged tickets was polluting the fiber grant ledger)** → `sonar_project_phase_map` → region/city lookup. `ask`-routed rows (GigaSpire adapters) then inherit the destination their same-account siblings agreed on — pure logic + tests in `lib/accountInheritance.js`; phase tag inherited only when donors agree, pre-filled picker as escape hatch; preview table clusters rows by account. When the wireless policy overrides a project tag, `phase_id` is left NULL so Sage cost-centers stay clean. Stamps `[sonar:<itemId>]` in notes + `consumed_by_user_id`/`phase_id` when known. **The `<itemId>` IS the physical asset tag** (first number of `Model Field Data \| Value List`, owner-confirmed Sep 2026); the importer also writes `line_note` = `Tag N · SN … · MAC … · Alt tag N` parsed by `lib/sonarAssetIds.js` (shared with `scripts/backfill-sonar-asset-tags.mjs`) so Activity / CSVs / By-account show which unit went in |
...
| **Reclassify** (`ReclassMovementSheet`, owner-only, from Activity / Reports → Consumption; also appended automatically by the fiber-jobs importer's backward fix-job check) | `transfer` with `reclass_of` | Region → Region | Sep 2026. Moves consumption between a grant project and its Service sibling after the fact. `buildReclassPayload` copies part / work date / account / installer / asset tag from the original and caps qty at what's left unreclassed (`getReclassChildren`). Reports subtract it from the old project; Sage gets one transfer line whose PROJECTID follows the destination phase. |
