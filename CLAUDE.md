...
- `public.users.crew_type` CHECK: `fiber_construction | splice | drop | locator | contractor | install | infrastructure | fiber_tech` (nullable) — plus legacy `aerial` / `underground`, kept VALID for back-compat but no longer assignable in the UI (June 2026 merged aerial + underground → `fiber_construction`; see Recent major work). Render via `crewTypeLabel()` in `lib/crewTypes.js`, never raw. Only **Crew** and **Working manager** carry a crew_type; every other staff type persists `crew_type = NULL`.
