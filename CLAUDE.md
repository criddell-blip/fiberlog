...
20. **🐛 BUG (high priority) — `recordMovementsBatch` drops `phase_id` + `consumed_by_user_id`.** Surfaced by the June 2026 code audit. The batch insert payload in `lib/inventory.js` (`recordMovementsBatch`, ~line 811-824) enumerates a fixed field list that omits both columns, but `SonarImportSheet.jsx` (`phase_id`/`consumed_by_user_id` at ~828/830) and `FiberJobsImportSheet.jsx` (~475/476) set them on every movement they pass in. Result: **every Sonar / fiber-jobs imported movement is written with `phase_id = NULL` and `consumed_by_user_id = NULL`** — silently losing the per-cost-center Sage rollup tag and crew attribution those importers exist to feed (ties into #4 Sage export + #0 "by consumer" reporting). Live-data evidence: 88 movements carry `[sonar…]` notes, only 32 have a non-null `phase_id`. Fix: add `phase_id: m.phase_id || null` and `consumed_by_user_id: m.consumed_by_user_id || null` to the payload map, then sanity-check the Sage export's project/phase join. ~5-min fix; the columns already exist.

21. **Code-audit follow-ups (June 2026).** Lower-priority items from the full-codebase audit; bucket A (accidental dead-code leftovers) already removed.
    - **Correctness/convention:** reset-password Save button enables at `pw.length < 6` but the policy is 8 (`AdminUsersView.jsx:~1147` → `< 8`); wrap the `.subscribe()` setup in try/catch in `SubmissionsQueue.jsx` + `CrewStatus.jsx` (only `unsubscribe` is guarded today); `AppContext.jsx:~203` `app_settings` channel uses `Math.random()` instead of `nextChannelSuffix()` and isn't try/caught; a `restricted_to_inventory` manager with a field `crew_type` can still flip into crew mode (`App.jsx:~63` doesn't check the restriction) — silent scope widening.
    - **Responsive:** `PurchaseRequestsTab.jsx` row grid is fixed-px (`GRID` ~line 22) and overflows at 390px — the one inventory tab that doesn't collapse to cards.
    - **Legacy whole-file removal candidates (bucket B — confirm before deleting):** `src/components/crew/UserSheet.jsx` (superseded by inline `SignOutConfirm`) and `src/UserPicker.jsx` (replaced by `Login.jsx`; its orphaned i18n keys `whoAreYou`/`pinComingSoon`/`searchName` go with it).
    - **Unreferenced-today, likely intentional scaffolding (bucket C — KEEP unless abandoned):** `clearSonarProjectPhase` / `clearSonarSourceLocation` / `clearFiberValueMap` (clear-siblings of used get/set pairs; relate to #14), `Icon` `ICON_NAMES`, `PartSearch` `filter` prop.
    - **Ambiguous — decide intent (bucket D):** `getPartsByCategory`, `getPendingSubmissions` (SubmissionsQueue uses an inline query instead), `ProjectManager` `projFiber` state.
    - **Noise:** stray `console.log`s in `SubmissionsQueue.jsx` / `ReportsView.jsx`; `PartSearch` search input missing `autoComplete="off"`; `InfraCrewApp` realtime `onInsert` defaults infra task type to `'aerial'`; no-op `ThemeToggle` pill in ManagerApp (dark Console deferred); `admin.js` relies on the internal `db.supabaseUrl`; stale doc note claiming `recordMovementsBatch` doesn't chunk (it does — see the chunking callers).
