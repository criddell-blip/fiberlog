...
| **Receipt reversal** (`reverse_pr_line_receipt` RPC, "↩ Reverse" on a received PR line) | `adjust` | Deliver-to/bin → NULL | Aug 2026, backlog #52. Credits a mis-receipt back to the PO: lowers `received_qty`, status recomputes (all lines 0 → `ordered`, lines unlock). Reason required; blocked when on-hand < qty. Activity labels a PR-linked adjust-down "Receipt reversal" (`movementDisplay`). Sage-neutral. PO receipts only — not field returns / found. |
