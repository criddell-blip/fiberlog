-- Guard: crew can never LOAD from a project (job_site) consumption bucket —
-- those are accumulate-only ledgers. Belt-and-suspenders behind the UI fix
-- that hides job_site as a movement source. Only the 'load' branch sources
-- from p_other_location_id; the rest source from the caller's own location.
-- Applied to the remote DB via MCP; kept here for parity. Only the new guard
-- in the load branch differs from the prior pull-location-aware definition.
...
BEGIN
  ...
  -- Resolve the caller's stock location: shared pull location first, else
  -- their active personal truck (mirrors getMyTruck).
  ...
  CASE p_operation
    WHEN 'load' THEN
      ...
      -- Project (job_site) consumption buckets are accumulate-only — never a
      -- valid pull source for crew.
      IF EXISTS (SELECT 1 FROM public.inventory_locations WHERE id = v_from AND type = 'job_site') THEN
        RAISE EXCEPTION 'Cannot load from a project bucket' USING ERRCODE = '42501';
      END IF;
